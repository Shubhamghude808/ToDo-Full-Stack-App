...
public class JwtSecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		return httpSecurity.csrf(AbstractHttpConfigurer::disable) // (1)
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers("/", // #CHANGE
						"/authenticate", "/actuator", "/actuator/*").permitAll().requestMatchers("/h2-console/**")
				...
				.headers(header -> header.frameOptions(frameOptionsConfig -> frameOptionsConfig.sameOrigin())).build();
	}
	...
	public UserDetailsService userDetailsService() {
		UserDetails user = User.withUsername("Shubham").password("{noop}git808").authorities("read").roles("USER")
				.build();
